...
// цикле (до 10 попыток — с двумя документами на сделку и ~10% отказа на
// документ это даёт эмпирическую вероятность добраться до нужного случая
// свыше 98%) и использует первую же сделку, на которой эмулятор
// действительно отклонил хотя бы один документ на первой проверке. Если за
// 10 попыток отказа не случилось — тест падает с объясняющим сообщением:
// это сигнал, что вероятность отказа в эмуляторе изменилась, а не что тест
// сломан.
...
// сотни, и на такой длине Playwright 1.63 falls over при финализации
// trace/video артефактов уже ПОСЛЕ того, как все ассерты теста прошли:
// `apiRequestContext.dispose: ENOENT ... traces/...recording*.trace` в
// dispose()/stop() — тест реально зелёный (see test-failed-1.png с
// "Завершена" при таком крэше), но раннер помечает его failed из-за
// исключения в teardown. Скриншота на failure достаточно для диагностики
// этого сценария (весь путь и так залогирован в самом тесте), поэтому для
// этого файла отключаем trace/video, а не тащим весь стенд под них.
...
test.describe("Восстановление сделки после отклонённого документа", () => {
  test("переподанный документ одобряется, сделка доходит до completed", async ({ page, tick }) => {
    // До 10 сделок подряд, и на каждую — реальное настенное ожидание двух
